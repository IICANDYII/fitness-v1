...
def build_dataset(
    ...
    for d in data_dirs:
        ...
        labeled_intervals = build_posture_labels_from_gt(gt, exercise_map, imu_df["time_sec"].iloc[-1])
...
def test_with_gt(
    ...
    for d in data_dirs:
        ...
        gt_segments = []
        for seg, (t0, t1, posture) in zip(gt, gt_intervals):
            gt_segments.append({
...
<div class="legend">"""]
    ...
    for r in results:
        ...
        for seg in r["gt_segments"]:
            left = seg["start_sec"] / T * 100
            w = max((seg["end_sec"] - seg["start_sec"]) / T * 100, 0.3)
        ...
        nt = min(int(T // 60) + 1, 12)
        for i in range(nt + 1):
            t = i * (T / nt)
            parts.append(f'<span style="left:{t/T*100:.1f}%">{fmtime(t)}</span>')
        parts.append('</div>')
